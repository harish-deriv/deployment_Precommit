...
invoke_test_3(){
    $git_path commit -m "test3" --no-verify
    if [ $? -ne 0 ]; then
...
invoke_test_5(){
    $git_path commit -m "Test5" --no-verify
    if [ $? -ne 0 ]; then
